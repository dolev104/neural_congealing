...
from utils_atlas.splat2d_cuda import splat2d
...
def save_video(frames, fps, out_path, filenames=False, codec='libx264', input_is_tensor=False, apply_normalize=True):
    if input_is_tensor:  # (T, C, H, W) in [0, 1] --> length-T list of (H, W, C) uint8 numpy array in [0, 255]
        if apply_normalize:
            frames = normalize(frames, 0, 1, inplace=False).mul_(255)
