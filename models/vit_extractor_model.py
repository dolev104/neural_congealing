...
# Added get_feat_extractor and changed preprocessing a bit
import argparse
import torch
...
def get_feat_extractor(config, device):
    return ViTExtractor(
        model_type=config["dino_model_type"], stride=config["dino_stride_size"], device=device
    )
...
class ViTExtractor:
    ...
    def __init__(self, model_type: str = 'dino_vits8', stride: int = 4, model: nn.Module = None, device: torch.device = torch.device("cuda")):
        """
        ...
        self.embedding_dim = self.model.patch_embed.proj.out_channels
    ...
    def normalize_image(self, input_image: torch.Tensor) -> torch.Tensor:
        """
        Normalize input image
        """
        prep = transforms.Normalize(mean=self.mean, std=self.std)
        normalized_img = prep(input_image.squeeze())[None, ...]
        return normalized_img

    def preprocess_pil_image(self, pil_image: Image.Image,
                   load_size: Union[int, Tuple[int, int]] = None) -> Tuple[torch.Tensor, Image.Image]:
        """
        Preprocesses an image before extraction.
        :param pil_image: PIL image.
        :param load_size: Size to resize image before the rest of preprocessing.
    ...
    def extract_descriptors(self, batch: torch.Tensor, layer: int = 11, facet: str = 'key',
                            bin: bool = False, include_cls: bool = False, binnig_hierarchy: int = 2) -> torch.Tensor:
        """
        ...
        if not bin:
            ...
            desc = self._log_bin(x, hierarchy=binnig_hierarchy)
        return desc

    def extract_descriptors_multi(self, batch: torch.Tensor, layers: List[int] = 11, facet: str = 'key',
                            bin: bool = False, include_cls: bool = False, binnig_hierarchy: int = 2) -> torch.Tensor:
        """
        extract descriptors from the model
        :param batch: batch to extract descriptors for. Has shape BxCxHxW.
        ...
        if not bin:
            ...
            desc = torch.cat([a.unsqueeze(0) for a in x], dim=-1)  # 1x1xtx(d x h x num_layers)
        else:
            desc = self._log_bin(x, hierarchy=binnig_hierarchy)
        return desc
    ...
    def extract_saliency_maps(self, batch: torch.Tensor) -> torch.Tensor:
        ...
        head_idxs = [0, 2, 4, 5]
        curr_feats = self._feats[0]  # Bxhxtxt [batch, heads, (H/p)*(W/p)+1, (H/p)*(W/p)+1]
