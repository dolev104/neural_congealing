...
def evaluate_model(args):
    ...
    evaluation_data = logger.evaluate_and_log(dataset, model, log_to_wandb=False, sim_only_bootstrap=(not model.with_flow), reduced_logging=False, eval_only_mode=True)
    ...
    curr_path.mkdir(parents=True, exist_ok=True)
    atlas_space_average_image = evaluation_data["atlas_vis/atlas_space_average_image"]
    ...
    atlas_keys_checkerboard = transforms.Resize(atlas_space_average_image.shape[-1])(atlas_keys_checkerboard).expand(-1, 3, -1, -1)
